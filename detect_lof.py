...
# ==== 可配置 ====
INPUT_CSV = "flows.csv"
OUTPUT_TOPK = "lof_top_anom.csv"
FEATURES = ["packets","bytes","duration","avg_iat","std_iat","pps"]
CONTAMINATION = 0.01   # 仅用于top-p阈值策略
N_NEIGHBORS = 35       # LOF邻居数（可调：20~50）
LEAF_SIZE = 30
METRIC = "minkowski"   # 可选 'euclidean' 等
SEED = 42
...
def main():
    ...
    # LOF 只在 fit_predict 时返回标签，但我们需要“异常分数”
    # sklearn 的 LOF 有 negative_outlier_factor_：越小越异常
    lof = LocalOutlierFactor(
        n_neighbors=N_NEIGHBORS,
        leaf_size=LEAF_SIZE,
        metric=METRIC,
        novelty=False  # 训练集上评估
    )
    _ = lof.fit_predict(Xs)
    neg_factor = lof.negative_outlier_factor_
    # 将“越小越异常”的负分数转为“越大越异常”的正分数
    scores = -neg_factor
    ...
    df.sort_values("lof_score", ascending=False).head(50).to_csv(OUTPUT_TOPK, index=False)
    print(f"Saved top LOF anomalies to {OUTPUT_TOPK}. Total={len(df)}")
